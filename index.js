...
const botInvoice = new Telegraf(process.env.TELEGRAM_BOT_TOKEN)
...
botInvoice.use(Telegraf.log())

botInvoice.start((ctx) => {
    const chatId = ctx.chat.id;
    return ctx.replyWithInvoice(getInvoice(chatId));
});


botInvoice.on('pre_checkout_query', (ctx) => ctx.answerPreCheckoutQuery(true)) 

botInvoice.on('successful_payment', async (ctx, next) => { 
