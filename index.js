...
const { walletMenuCallbacks } =  require('./src/connect-wallet-menu');
const {
    handleConnectCommand,
    handleDisconnectCommand,
    handleSendTXCommand,
    handleShowMyWalletCommand
} = require('./src/commands-handlers.js');
const { initRedisClient } =  require('./src/ton-connect/storage');
...
bot.on("callback_query", async (callbackQuery) => {
    ...
    if (data === "settings") {
        const qrCode = qr.imageSync(`https://t.me/Trood_2024_Bot`, {
            type: "png",
        });
        ...
        bot.sendMessage(
            chatId,
            "Enter a name for your wallet (use /create YOUR_NEW_NAME_WALLET):"
        );
    } else if (data === "delete") {
        bot.sendMessage(
            chatId,
            Object.values(walletNames).length > 0
                ? `Which wallet do you want to delete: ${Object.values(
                      walletNames
                  ).join(", ")}`
    ...
    if (msg?.web_app_data?.data) {
        ...
        try {
            ...
            if (data.totalTime > 0) {
                bot.sendMessage(
                    chatId,
                    `You spent ${data.totalTime} ${
                        data.totalTime > 1 ? "hours" : "hour"
                    }`
                );
            }
...
bot.on("message", async (msg) => {
    ...
    if (text === "/start") {
        await bot.sendMessage(
            chatId,
            `Hi ${nickname}, welcome to telegram bot. My name is TroodBot.`
        );
        setTimeout(() => {
            bot.sendMessage(
                chatId,
                "I'm a bot that helps people track time, and also take into account the user's health status based on their images."
            );
        }, 1000);
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                bot.sendMessage(
                    chatId,
                    "What is your name, surname, year of birth? (Please use commas to separate data)"
                );
            }, 1000);
        }, 3000);
        ...
        if (userData?.length !== 3) {
            bot.sendMessage(
                chatId,
                "Please provide your name, surname, and year of birth separated by commas."
            );
            return;
        }
        ...
        if (isNaN(age) || age <= 0 || age >= 90) {
            bot.sendMessage(
                chatId,
                "Invalid year of birth. Please provide a valid year of birth."
            );
            return;
        }
        ...
        bot.sendMessage(
            chatId,
            `Are you ${surname} ${name} ${age} years old ?`
        );
        setTimeout(() => {
            bot.sendMessage(
                chatId,
                "Please fill the form below, and make a photo",
                {
                    reply_markup: {
                        keyboard: [
